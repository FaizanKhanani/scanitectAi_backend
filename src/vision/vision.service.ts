...
//   // Mirror config (set to your endpoint that returns a final /files/<id> URL)
//   // If your API expects a different route or auth, adjust below.
//   private readonly FILES_MIRROR_URL = process.env.FILES_MIRROR_URL || 'https://scanitectai.com/api/files/mirror';
//   private readonly FILES_AUTH_TOKEN = process.env.FILES_AUTH_TOKEN || ''; // e.g., 'Bearer <token>' if needed
...
//   // Pulls lots of images attached to the page, including URLs and thumb URLs
//   private async fetchPageGallery(pageTitle: string, lang = 'en', max = 50) {
//     const res = await this.wiki.get(`https://${lang}.wikipedia.org/w/api.php`, {
//       params: {
//         action: 'query',
//         generator: 'images',
...
//   // Heuristic: drop logos, flags, seals, svg, maps, tiny or extreme aspect
...
//     if (w < 600 || h < 400) return true; // too small for a decent building photo

//     const ratio = w && h ? Math.max(w, h) / Math.min(w, h) : 1;
//     if (ratio > 3.5) return true; // extreme aspect ratio often not a photo

//     return false;
//   }

//   // Prefer images that look like the page subject name, large, landscape
//   private scoreForBuilding(img: {
//     title?: string;
//     width?: number;
//     height?: number;
//   }, pageName: string) {
...
//     const w = img.width || 0;
//     const h = img.height || 0;
//     score += Math.min(30, Math.floor((w * h) / (1200 * 800)) * 2); // size bonus
//     if (w >= h) score += 4; // landscape slight bonus
//     return score;
//   }

//   private async mirrorImageToLocal(remoteUrl: string, filename?: string): Promise<string | null> {
//     try {
//       // Option A: mirror by URL (preferred if your API supports it)
//       // POST { url, filename } → { url: 'https://scanitectai.com/api/files/<id>' }
...
//     } catch {
//       // Option B (fallback): download and multipart upload.
//       // Uncomment and adapt if your API expects multipart "file".
//       /*
//       try {
//         const fileResp = await axios.get(remoteUrl, { responseType: 'arraybuffer' });
//         const form = new FormData();
//         form.append('file', Buffer.from(fileResp.data), filename || 'image.jpg');

//         const up = await axios.post(
//           'https://scanitectai.com/api/files', // adjust if different
//           form,
//           {
//             headers: {
//               ...(this.FILES_AUTH_TOKEN ? { Authorization: this.FILES_AUTH_TOKEN } : {}),
...
//           }
//         );
//         return up.data?.url || up.data?.data?.url || null;
//       } catch (e2) {
//         console.warn('Multipart upload also failed:', (e2 as any)?.message || e2);
//       }
//       */
//       return null;
//     }
...
//       // Coordinates fallback from summary
//       const coords =
//         wd.coordinates ||
//         (wikiData.coordinates ? { lat: wikiData.coordinates.lat, lon: wikiData.coordinates.lon } : null);

//       // Build image set: start from summary image, then more from gallery
//       const baseName = wikiData.title || resolvedTitle;

//       // 1) Page gallery (many images)
//       let gallery = await this.fetchPageGallery(resolvedTitle, lang, 60);
//       // 2) Filter out logos/bad ones
//       gallery = gallery.filter(img => !this.looksBadForBuilding(img));
//       // 3) Rank by relevance
//       gallery.sort((a, b) => this.scoreForBuilding(b, baseName) - this.scoreForBuilding(a, baseName));
//       // 4) Keep top N
//       const topGallery = gallery.slice(0, 10);

//       // Lead image: prefer summary original if not a logo; otherwise from gallery
...
//         ? summaryLead
//         : topGallery[0] || summaryLead || null;

//       // Mirror the lead + gallery to your files service
//       let localLeadOriginal: string | null = null;
//       let localLeadThumb: string | null = null;
//       if (lead?.original) {
//         localLeadOriginal = await this.mirrorImageToLocal(lead.original);
//       }
//       if (lead?.thumb) {
//         localLeadThumb = await this.mirrorImageToLocal(lead.thumb);
//       }
...
//           local_thumbnail: localLeadThumb || '',
//           local_original: localLeadOriginal || '',
//           // extra images
//           gallery: galleryWithLocal
//         },
//         coordinates: coords,
//         wikidata: wd
//       };

...
// }
 ...
 async uploadMediaFromBuffer(
  ...
  // Inside VisionService class
// private isLabelCompatible(candidate: string | null, bestTitle?: string): boolean {
//   if (!candidate || !bestTitle) return false;
...
//   const labelTokens = new Set(normalize(candidate));
//   const titleTokens = new Set(normalize(bestTitle));

//   if (!labelTokens.size || !titleTokens.size) return false;
...
//   // at least 1 shared “real” word => considered compatible
//   return overlap >= 1;
// }
...
//   // We still compute this the same way, but we may ignore related label later
...
//     // 3) Pick best visual match WITHOUT using label at all
//     const candidates = visualMatches.slice(0, 40);
//     candidates.sort(
//       (a, b) =>
//         this.scoreVisualMatch(b, null) - this.scoreVisualMatch(a, null),
//     );
//     const best = candidates[0];

//     // 4) Decide label: trust KG / related_content only if compatible with best.title
//     let chosenLabel: string | null = null;

//     if (kgTitle && this.isLabelCompatible(kgTitle, best?.title)) {
//       chosenLabel = kgTitle;
//     } else if (
//       relatedQuery &&
//       this.isLabelCompatible(relatedQuery, best?.title)
//     ) {
//       // Only use related_content if it matches the visual cluster
//       chosenLabel = relatedQuery;
//     }

//     // 5) Fallback: derive label from best match title if KG/related are not usable
//     if (!chosenLabel) {
//       chosenLabel =
//         best?.title ||
//         best?.name ||

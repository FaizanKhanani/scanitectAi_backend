...
// Replace your entire 'recognize' method in your controller file with this.

// FILE 2: Your Controller File
// Replace the entire 'recognize' method with this.
...
async recognize(
 ...
 let buf: Buffer | undefined;
  ...
  // --- THIS IS THE CRUCIAL FIX (THE "TYPE GUARD") ---
  // We MUST check the 'status' property first to prove the type to TypeScript.
  ...
  // --- THIS CODE IS NOW 100% TYPE-SAFE ---
  // Because the `if` block above stops the function on failure,
  // TypeScript is now smart enough to know that from this point forward,
  // `results` MUST be a `SuccessRecognition` object.
  // The error on `results.data` will now be gone.
  ...
  throw new BadRequestException('Failed to get landmark details.');
